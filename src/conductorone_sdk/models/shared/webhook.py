"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from conductorone_sdk.types import (
    BaseModel,
    Nullable,
    OptionalNullable,
    UNSET,
    UNSET_SENTINEL,
)
import pydantic
from pydantic import model_serializer
from typing import Any, Dict, Optional
from typing_extensions import Annotated, NotRequired, TypedDict


class WebhookTypedDict(TypedDict):
    r"""The Webhook message.

    This message contains a oneof named webhook_identifier. Only a single field of the following list may be set at a time:
    - webhookId
    - webhookIdCel

    """

    payload: NotRequired[Dict[str, Any]]
    webhook_id: NotRequired[Nullable[str]]
    r"""The webhookId field.
    This field is part of the `webhook_identifier` oneof.
    See the documentation for `c1.api.automations.v1.Webhook` for more details.
    """
    webhook_id_cel: NotRequired[Nullable[str]]
    r"""The webhookIdCel field.
    This field is part of the `webhook_identifier` oneof.
    See the documentation for `c1.api.automations.v1.Webhook` for more details.
    """


class Webhook(BaseModel):
    r"""The Webhook message.

    This message contains a oneof named webhook_identifier. Only a single field of the following list may be set at a time:
    - webhookId
    - webhookIdCel

    """

    payload: Optional[Dict[str, Any]] = None

    webhook_id: Annotated[OptionalNullable[str], pydantic.Field(alias="webhookId")] = (
        UNSET
    )
    r"""The webhookId field.
    This field is part of the `webhook_identifier` oneof.
    See the documentation for `c1.api.automations.v1.Webhook` for more details.
    """

    webhook_id_cel: Annotated[
        OptionalNullable[str], pydantic.Field(alias="webhookIdCel")
    ] = UNSET
    r"""The webhookIdCel field.
    This field is part of the `webhook_identifier` oneof.
    See the documentation for `c1.api.automations.v1.Webhook` for more details.
    """

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = ["payload", "webhookId", "webhookIdCel"]
        nullable_fields = ["webhookId", "webhookIdCel"]
        null_default_fields = []

        serialized = handler(self)

        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k)
            serialized.pop(k, None)

            optional_nullable = k in optional_fields and k in nullable_fields
            is_set = (
                self.__pydantic_fields_set__.intersection({n})
                or k in null_default_fields
            )  # pylint: disable=no-member

            if val is not None and val != UNSET_SENTINEL:
                m[k] = val
            elif val != UNSET_SENTINEL and (
                not k in optional_fields or (optional_nullable and is_set)
            ):
                m[k] = val

        return m
