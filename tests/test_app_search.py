...
class TestAppSearch:
    """Test cases for AppSearch functionality."""
    
    def test_search_should_return_status_200_and_valid_response(self):
        """Test that search returns status 200 and valid response."""
        req = shared.SearchAppsRequest(page_size=10)
        res = sdk_instance.app_search.search(request=req)
        
        assert res is not None
        assert res.status_code == 200
        assert res.search_apps_response is not None
    
    def test_search_with_page_size(self):
        """Test app search with specific page size."""
        req = shared.SearchAppsRequest(page_size=5)
        res = sdk_instance.app_search.search(request=req)
        ...
        response_data = res.search_apps_response
